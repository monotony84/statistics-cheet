...
|用語|意味|
|:---|:---|
|反復 / replication|複数の処理を比較する際に、それぞれの処理に対して同じ条件で2回以上繰り返し実験（評価）を行うこと|
|無作為化 / randomization|実験の順序や場所などが複数ある場合に、比較したい処理群を無作為に割り付けること|
|局所管理 / local Control|実験を行う時間や場所を区切ってブロックを作り、そのブロック内でのバックグラウンドができるだけ均一になるように管理すること|
...
|:---|:---|
|帰無仮説|検定において否定したい仮説|
|対立仮説|検定において正しいことを期待したい仮説|
|有意水準|めったに起こらないと言えるかどうかの判断基準となる確率。一般的には5％|
...
|検出力|対立仮説が正しいとき、帰無仮説を棄却できる確率。第二種の過誤を犯さない確率|
